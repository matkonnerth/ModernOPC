...
static UA_StatusCode getMem(Value *val)
{
    ...
    if (val->value)
    {
        memcpy(newVal, val->value,
               val->typestack->type->memSize * val->arrayCnt);
        UA_free(val->value);
    }
    val->value = newVal;
...
Value *Value_new(const TNode *node)
{
    ...
    UA_NodeId dataTypeId =
        getNodeIdFromChars(((TVariableNode *)node)->datatype);
        ...
        if (!strcmp(val->name, localname))
        {
            ...
            if (val->typestack->type->members)
            {
                ...
                val->typestack = TypeList_push(
                    val->typestack, &UA_TYPES[val->typestack->type->members[idx]
                                                  .memberTypeIndex]);
                val->state = VALUE_STATE_DATA;
                if (val->typestack->type->members)
                {
...
static void setBoolean(uintptr_t adr, char *value)
{
    *(UA_Boolean *)adr = isTrue(value);
}

static void setSByte(uintptr_t adr, char *value)
{
    *(UA_SByte *)adr = (UA_SByte)atoi(value);
}

static void setByte(uintptr_t adr, char *value)
{
    *(UA_Byte *)adr = (UA_Byte)atoi(value);
}

static void setInt16(uintptr_t adr, char *value)
{
    *(UA_Int16 *)adr = (UA_Int16)atoi(value);
}

static void setUInt16(uintptr_t adr, char *value)
{
    *(UA_UInt16 *)adr = (UA_UInt16)atoi(value);
}

static void setInt32(uintptr_t adr, char *value)
{
    *(UA_Int32 *)adr = atoi(value);
}

static void setUInt32(uintptr_t adr, char *value)
{
    *(UA_UInt32 *)adr = (UA_UInt32)atoi(value);
}

static void setInt64(uintptr_t adr, char *value)
{
    *(UA_Int64 *)adr = atoi(value);
}

static void setUInt64(uintptr_t adr, char *value)
{
    *(UA_UInt64 *)adr = (UA_UInt64)atoi(value);
}

static void setFloat(uintptr_t adr, char *value)
{
    *(UA_Float *)adr = (UA_Float)atof(value);
}

static void setDouble(uintptr_t adr, char *value)
{
    *(UA_Double *)adr = atof(value);
}

static void setString(uintptr_t adr, char *value)
{
...
static void setDateTime(uintptr_t adr, char *value)
{
    printf("DateTime: %s currently not handled\n", value);
}

static void setNodeId(uintptr_t adr, char *value)
{
    // todo: namespaceIndex should be converted?
    *(UA_NodeId *)adr = extractNodeId(value); // getNodeIdFromChars(value);
}

static void setNotImplemented(uintptr_t adr, char *value)
{
...
static void setScalarValue(Value *val, const UA_DataType *type, char *value)
{
...
static void nextType(Value *val)
{
    ...
    if (val->typestack->memberIndex >=
        (size_t)val->typestack->type->membersSize - 1)
    {
        return;
    }
