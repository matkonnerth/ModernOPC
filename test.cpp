...
UA_UInt64 readNanosecondsValueLoop(UA_Client *client)
{
    UA_StatusCode retval = UA_Client_readValueAttribute(
        client, UA_NODEID_NUMERIC(1, 42), &readNanosecondsVariant);
    if (retval == UA_STATUSCODE_GOOD &&
        UA_Variant_isScalar(&readNanosecondsVariant) &&
    ...
    {
        printf("ERROR: Could not read variable: %s",
...
int main(int argc, char *argv[])
{
    ...
    while (running)
    {
        ...
        if (counter == 100000)
        {
            ...
            nsMin = UA_UINT32_MAX;
            nsMax = 0;
            // printing the log takes some time. Reset values to ensure correct
            // measuring
            clock_gettime(CLOCK_MONOTONIC, &tNow);
            lastPrintTime =
                (double)tNow.tv_sec + (double)tNow.tv_nsec / 1000000000.0;
